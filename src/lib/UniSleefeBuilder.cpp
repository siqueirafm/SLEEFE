...
namespace sleefe {
UniSleefe UniSleefeBuilder::build(int numberOfSegments,
  ...
  if (numberOfSegments < 1 || numberOfSegments > MaximumNumberOfSegments) {
    throw std::invalid_argument("The number of segments of the sleefe must "
                                "be greater than 0 and no more than " +
  ...
  // Compute the values of the lower and upper sleefe components at the
  // breakpoints.
  ...
  for (auto segIdx = 0; segIdx <= numberOfSegments; ++segIdx) {
    ...
    // Compute the linear contribution to the component values of the sleefe.
  ...
  return UniSleefe(lowerValues, upperValues);
}

double UniSleefeBuilder::aerp(double t, double a, double b) const {
  if (t < 0.0 || t > 1.0) {
