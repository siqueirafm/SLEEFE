...
UniSleefe::UniSleefe(const std::vector<double> &lower,
...
double UniSleefe::lowerValueAt(double t) const {
  if (t < 0.0 || t > 1.0) {
...
double UniSleefe::upperValueAt(double t) const {
  if (t < 0.0 || t > 1.0) {
    throw std::invalid_argument(
        "The parameter value must be a number in the real line interval [0,1]");
  }
...
const std::vector<double> &UniSleefe::lowerValues() const { return _lower; }

const std::vector<double> &UniSleefe::upperValues() const { return _upper; }

int UniSleefe::numberOfSegments() const { return _numberOfSegments; }
